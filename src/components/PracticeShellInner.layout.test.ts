...
describe('practice shell full-page layout', () => {
  ...
  it('uses the approved dictation report hierarchy and copy', () => {
    ...
    expect(source).toContain('class="dictationReportTop"')
    expect(source).toContain('class="dictationReportDock"')
    expect(source).toContain('class="dictationReportScrollContent"')
    expect(source).toContain('点击单词右侧按钮可切换掌握情况')
    expect(source).toContain("item.isForgotten ? '待巩固' : '掌握'")
    expect(source).not.toContain('待巩固的词会留在生词本，之后可再听一轮。')
    expect(source).not.toContain('点击右侧可切换结果')
  ...
  it('uses the approved reward hierarchy and actions', () => {
    ...
    expect(source).toContain('--burst-rotate:')
    expect(source).not.toContain('@keyframes rewardBurstRing')
    expect(source).not.toContain('rewardConfettiLayer::before')
  ...
  it('keeps the home hierarchy balanced instead of stretching the daily word', () => {
    ...
    expect(source).toContain('class="homeFeedbackButton"')
    expect(source).toContain('aria-label="Feedback"')
    expect(source).toContain('class="homeFeedbackDot"')
    expect(source).not.toContain('homeFeedbackLabel')
    expect(source).toContain('flex: 0 0 44px;')
    expect(source).toContain('--home-action-right-inset:')
    expect(source).toContain('windowWidth - menuLeft - 8')
    expect(source).toContain('padding-right: var(--home-action-right-inset, 88px);')
    expect(source).toContain("url: '/pages/feedback/index'")
