...
const showBottomNav = computed(() => {
  // #ifdef MP-WEIXIN
  // Tab pages stay mounted in the stack; only the visible shell may host fixed chrome.
  if (!shellVisible.value) return false
  if (props.routeScreen) return false
  if (!TAB_ROOT_SCREENS.has(activeScreen.value)) return false
  return tabRootScreen.value === activeScreen.value
  // #endif
  return false
})
...
function canSyncNativeTabBar(): boolean {
  // Sub-pages (course, word-detail, ...) never host the native tab bar.
  if (props.routeScreen) return false
  // Tab shells stay mounted in the page stack but must not touch native chrome while hidden.
  if (!shellVisible.value) return false
...
function isHostingPageActive(): boolean {
  const currentPath = getCurrentRoutePath()
  if (props.routeScreen) {
    const routePath = ROUTE_BY_SCREEN[props.routeScreen]
    return Boolean(routePath && currentPath === routePath)
  }
...
function isNativeTabBarPage(): boolean {
  return NATIVE_TAB_BAR_ROUTES.has(getCurrentRoutePath())
}
...
watch(
  () => [shellVisible.value, showBottomNav.value, activeScreen.value],
  () => {
    syncNativeTabBar()
  },
  { immediate: true }
...
watch(
  () => screen.value,
  nextScreen => {
    if (!props.routeScreen || !shellVisible.value) return
...
onBeforeUnmount(() => {
  ...
  destroyActiveAudio()
  // #ifdef MP-WEIXIN
  if (props.routeScreen || !props.tabScreen) return
...
.dictationSetupScreen .pill text {
  font-size: 14px;
  line-height: 1;
  white-space: nowrap;
}
