<template>
  <view :class="['screen', showBottomNav && 'hasBottomNav']">
    ...
    <view v-if="screen === 'home'" class="sectionStack homeScreen">
      <view class="heroBlock">
        <view class="heroTitleRow">
          <text class="heroTitle">{{ selectedUnit?.publisherName }} {{ selectedUnit?.bookName }} Unit {{ selectedUnit?.unitNumber }}</text>
        </view>

        <view class="heroStats">
          <view class="heroStatCard" @tap="openUnitWords">
            <text class="heroStatValue">{{ unitWordCount }}</text>
            <text class="heroStatLabel">总单词</text>
          </view>
          <view class="heroStatCard">
            <text class="heroStatValue">{{ unitMasteryLabel }}</text>
            <text class="heroStatLabel">已掌握</text>
          </view>
        </view>
        ...
        <view class="heroHint" @tap="openUnitWords">
          <text>点总单词查看全表，标记认识后不进入体检和听写</text>
        </view>
      </view>
      ...
      <view class="practiceList">
        <view class="practiceItem isPrimary" @tap="openCheckupSetup">
          <view class="practiceCopy">
            <text class="practiceType">先测</text>
            <text class="practiceTitle">词汇体检</text>
            <text class="practiceDesc">识义后立刻拼写，只找真正不会的词</text>
          </view>
        ...
        <view class="practiceItem" @tap="openDictationSetup">
          <view class="practiceCopy">
            <text class="practiceType blue">听写</text>
            <text class="practiceTitle">自动听写</text>
            <text class="practiceDesc">支持听英文和听中文，在线输入或纸笔默写</text>
          </view>
          <view class="practiceAction dark">
            <text>设置</text>
          </view>
        </view>
      </view>
    ...
    <view v-else-if="screen === 'checkupSetup'" class="checkupSetupScreen">
      ...
      <view class="checkupLimitOptions">
        <view
          ...
          :key="option"
          :class="['checkupLimitOption', effectiveCheckupLimit === option && 'isActive']"
          @tap="setCheckupLimit(option)"
        >
      ...
      <view class="dictationStartButton" @tap="startCheckup">
        <text>开始体检</text>
      </view>
    </view>
    ...
    <view v-else-if="screen === 'dictationSetup'" class="dictationSetupScreen">
      ...
      <view class="dictationIntro">
        ...
        <text class="dictationIntroText">手机负责报词，孩子继续用纸笔完成。</text>
      </view>
      ...
      <view v-if="dictationPrompt === 'english'" class="settingGroup">
        <text class="settingLabel">发音</text>
        <view class="pillRow">
          <view :class="['pill', dictationAccent === 'uk' && 'isActive']" @tap="dictationAccent = 'uk'">
            <text>英音</text>
          </view>
          <view :class="['pill', dictationAccent === 'us' && 'isActive']" @tap="dictationAccent = 'us'">
            <text>美音</text>
          </view>
        </view>
      </view>
      ...
      <view v-if="dictationMode === 'online'" class="dictationTip">
        <text>在线输入会记录对错并同步生词本。</text>
      </view>
    ...
    <view v-else-if="screen === 'dictation' && currentDictationEntry" class="dictationPlayerScreen">
      ...
      <view v-if="dictationMode === 'online'" class="onlineAnswerPanel">
        ...
        <view v-if="showDictationAnswer" :class="['answerBox', lastDictationCorrect ? 'success' : 'danger']">
          ...
          <text class="feedbackText">{{ currentDictationEntry.word }} / {{ currentDictationEntry.meaning }}</text>
        </view>
        <view
    ...
    <view v-else-if="screen === 'dictationReport'" class="sectionStack reportScreen">
      <view class="flowHeader">
        <view class="backButton" @tap="resetPractice">返回</view>
        <view class="flowTitle">
          <text class="flowTitleText">听写报告</text>
          <text class="flowMeta">{{ dictationMode === 'paper' ? '已完成' : `${dictationSummary.correct}/${dictationSummary.total}` }}</text>
        </view>
      </view>
      ...
      <view class="reportHero">
        <view class="reportTopline">
          <view class="reportCopy">
            ...
            <text class="reportText">
              {{ dictationMode === 'paper' ? `完成 ${dictationSummary.total} 个词，纸笔默写不记录对错。` : dictationReportText }}
            </text>
          </view>
        </view>

        <view class="scoreVisual">
          <view class="scoreNumberBlock">
            <text class="reportScore">{{ dictationMode === 'paper' ? dictationSummary.total : `${dictationSummary.accuracy}%` }}</text>
            <text class="scoreBadgeLabel">{{ dictationMode === 'paper' ? '完成词数' : '正确率' }}</text>
          </view>
          <view class="scoreMeter">
            <view v-if="dictationMode === 'online'" class="masteryRail">
              <view class="railSegment mastered" :style="{ width: dictationCorrectSegmentWidth }" />
              <view class="railSegment weak" :style="{ width: dictationWrongSegmentWidth }" />
            </view>
            <view class="scoreLegend">
              <view class="legendRow mastered">
                <text class="legendName">{{ dictationMode === 'paper' ? '已完成' : '拼对' }}</text>
                <text class="legendValue">{{ dictationMode === 'paper' ? dictationSummary.total : dictationSummary.correct }}</text>
              </view>
              <view v-if="dictationMode === 'online'" class="legendRow weak">
                <text class="legendName">需订正</text>
                <text class="legendValue">{{ dictationSummary.wrong }}</text>
              </view>
            </view>
          </view>
        </view>
      ...
      <view class="weakList">
        <text class="blockTitle">{{ dictationMode === 'paper' ? '答案订正' : '本次错词' }}</text>
        <view v-if="dictationMode === 'paper'">
          <view v-for="word in paperAnswerPreviewWords" :key="word.id" class="weakRow">
            <text class="weakWord">{{ word.word }}</text>
            <text class="weakMeaning">{{ word.meaning }}</text>
          </view>
          <text v-if="paperAnswerHiddenCount > 0" class="weakMoreText">还有 {{ paperAnswerHiddenCount }} 个，可在生词本继续处理</text>
        </view>
        <view v-else-if="dictationWrongWords.length === 0" class="emptyState">
          <text>本次全部拼对，相关生词会自动移出生词本。</text>
        </view>
        <view v-else>
          <view v-for="word in dictationWrongPreviewWords" :key="word.id" class="weakRow">
            <text class="weakWord">{{ word.word }}</text>
            <text class="weakMeaning">{{ word.meaning }}</text>
          </view>
          <text v-if="dictationWrongHiddenCount > 0" class="weakMoreText">还有 {{ dictationWrongHiddenCount }} 个，已放入生词本</text>
        </view>
      </view>
...
const checkupEstimateMinutes = computed(() => {
  if (effectiveCheckupLimit.value <= 0) return 0
  return Math.max(1, Math.ceil(effectiveCheckupLimit.value / 4))
})
...
const paperAnswerPreviewWords = computed(() => paperAnswerWords.value.slice(0, 5))

const paperAnswerHiddenCount = computed(() => Math.max(0, paperAnswerWords.value.length - paperAnswerPreviewWords.value.length))

const dictationWrongPreviewWords = computed(() => dictationWrongWords.value.slice(0, 5))

const dictationWrongHiddenCount = computed(() => Math.max(0, dictationWrongWords.value.length - dictationWrongPreviewWords.value.length))
...
const dictationReportText = computed(() => {
  if (dictationSummary.value.wrong === 0) return '本次全部拼对，继续保持。'
  return `已记录 ${dictationSummary.value.wrong} 个需订正单词。`
})

const dictationCorrectSegmentWidth = computed(() => {
  return getSegmentWidth(dictationSummary.value.correct, dictationSummary.value.total)
})

const dictationWrongSegmentWidth = computed(() => {
  return getSegmentWidth(dictationSummary.value.wrong, dictationSummary.value.total)
})
...
const paperAnswerWords = computed(() => dictationPlan.value?.words ?? [])

const showBottomNav = computed(() => {
  return screen.value === 'home' || screen.value === 'weakbook'
})
...
function onCheckupLimitInput(event: Event) {
  ...
  setCheckupLimit(Number(miniProgramValue ?? webValue ?? ''))
}
...
watch(
  () => [screen.value, currentDictationEntry.value?.id, dictationAudioUrl.value, dictationAudioReady.value, dictationMode.value],
  () => {
    if (screen.value !== 'dictation') {
      clearDictationTimers()
      destroyActiveAudio()
